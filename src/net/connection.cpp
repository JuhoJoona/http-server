...
bool Connection::isClosed() const { return closed; }
...
void Connection::registerWithEventLoop() {
  if (!eventLoop.addSocket(sockfd, ConnectionEvent::READ)) {
    std::cerr << "Failed to register client with event loop" << std::endl;
    close();
  }
}

void Connection::unregisterFromEventLoop() { eventLoop.removeSocket(sockfd); }
